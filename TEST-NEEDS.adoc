...
* [x] `+manifest::init_manifest+` — creates `+idrisiser.toml+` in a temp
dir
* [x] `+manifest::load_manifest+` + `+validate+` — roundtrip against
example manifest
* [x] `+validate+` rejects empty project name
* [x] 31 unit test annotations across source modules (codegen, ABI,
etc.)
...
* Integration tests depend on `+examples/user-api/idrisiser.toml+` — if
that example is removed, tests break
* No property-based tests

=== Still Missing (for CRG B+)

* [ ] Fuzz targets for manifest parsing
* [ ] Tests for the Idris2 codegen output (verify generated `+.idr+`
files)
